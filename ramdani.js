...
module.exports = Ramdani = async (Ramdani, mek) => {
	try {
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Ramdani.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "© ʙʏ Ȥ✗¹²","listType": "SINGLE_SELECT","sections": list}}, {})
...
"externalAdReply": {
      ...
      const demoteAdmin = async function(to, target=[]){
          ...
          if (!packname) packname = 'Dibuat Oleh'; if (!author) author = '© Ȥ✗¹²';author = author.replace(/[^a-zA-Z0-9]/g, '');	
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/foto/thumb.jpg`)},"title": `© ʙʏ ʀᴀᴍᴅᴀɴɪ ᴏғғɪᴄɪᴀʟ`,"description": "Ȥ✗¹²", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "Ȥ✗¹²","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
...
danzrun = process.uptime() 
           Ramdani.setStatus(`${namabot} Aktif Selama ${(danzrun)} © Creator By Ȥ✗¹²`).catch((_)=>_); //DI UBAH YATIMM
...
break
       ...
       if (isBanned) return freply(mess.banned)
               ...
               buttons = [{buttonId: `${prefix}gopay`,buttonText:{displayText: ''},type:1},{buttonId: `${prefix}dana`,buttonText:{displayText: 'DANA'},type:1},{buttonId: `${prefix}qris`,buttonText:{displayText: ''},type:1}]
...
> 532 ${prefix}nsfw_avatar`
               ...
               buttonsMessage = {
               contentText: `${allmenu}`,
               footerText: '© ʙʏ Ȥ✗¹²', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
case 'tiktok':
              ...
              buttonsMessage = {footerText:'Ȥ✗¹² ganteng tq:v, imageMessage: imageMsg,
       ...
       case 'img2url':
         ...
         case 'asupan':
              ...
              .then(async (body) => {
               ...
               sendMediaURL(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, '\`\`\`ASUPAN BRO GUA DAPAT DARI Ȥ✗¹²\`\`\`')
...
case 'ttp':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              if (args.length == 0) return freply(`Example: ${prefix + command} Ȥ✗¹²`)
              woy = args.join(" ")
              freply(mess.wait)
...
//------------------<18+ Menu>-----------------------   
       case 'randombokep':
              ...
              Ramdani.sendMessage(from, '*PERMINTAAN:* '+bokep+'\n*DOSA TANGGUNG PRIBADI*\n*NI BRO FREE BUAT KAMU DOWNLOAD SENDIRI:* '+ kep, text, { quoted: ftoko, caption: `NI BOKEP SAYA DAPAT DARI *Ȥ✗¹²* DOSA TANGGUNG SENDIRI🗿`})
              break
                case 'xnxx':
              if (!isRegist) return freply(mess.regist)
              if (isDewasa) return freply(mess.dewasa)
...
Ramdani.sendMessage(from, tapib1, document, { quoted: mek, filename:`Ȥ✗¹² ~ 404 ${danz}.mp3`, mimetype: 'audio/application' })
...
//------------------< Lainnya >-------------------
      ...
      case 'caripesan':
       ...
       case 'bugreport':
              ...
              Ramdani.sendMessage('6282137435046@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
